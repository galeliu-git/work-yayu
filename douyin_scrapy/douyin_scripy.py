...
def craw_comment_info_by_url(url):
    ...
    page = ChromiumPage(co)
    data_path_comment_info = 'aweme/v1/web/comment/list/'
    page.listen.start(targets=data_path_comment_info)
    page.get(url)
    # print(page.mode)
    comment_data_list = []
    i = 0
    ...
    for i in range(20):
        ...
        for comment in comment_info_json_data['comments']:
            # 评论人
...
def main_comment_info():
    keyword = '背债'
    save_url_path = r'video_url_{}.txt'.format(keyword)
    # 评论信息存储路径
    comment_info_path = r'comment_info_{}.csv'.format(keyword)
    ...
    comment_info_columns = ['video_url', 'comment_user',
                            'comment_text', 'comment_time', 'comment_location', 'comment_user_url']
